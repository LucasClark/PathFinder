        ...
        public List<Tile> FindPath(List<List<TileObject>> myList, TileObject startPoint, TileObject exitPoint, Vector2 gridLength)
        {
            ...
            while (stopper < stop)//while there are unchecked nodes or while the stoppper is below its allowed limit
            {
                for (int i = 0; i < searchType; i++)// if its 4 it only checks up down left right. if its 8 it goes diagonal as well
                {
                    ...
                    if (tileToCheck.x < gridLength.x && tileToCheck.x >= 0 && tileToCheck.y < gridLength.y && tileToCheck.y >= 0)// if the tile is not inside the grid skip it
                    {
                        ...
                        if (myList[(int)cT.x][(int)cT.y].EstimatedCost + moveCost < tileBeingChecked.EstimatedCost && tileBeingChecked.HasBeenChanged == false)//checks if the cost of the current tile is less than the current lowest cost to get to that tile
                        {
                            tileBeingChecked.EstimatedCost = myList[(int)cT.x][(int)cT.y].EstimatedCost + moveCost;// sets the cost of the tile by adding the movement cost to the tile from the current tile
            ...
            while (stopper < stop)//while there are unchecked nodes or while the stoppper is below its allowed limit
            {
                for (int i = 0; i < searchDirections; i++)// if its 4 it only checks up down left right. if its 8 it goes diagonal as well
                {
                    ...
                    if (tileToCheck.x < gridLength.x && tileToCheck.x >= 0 && tileToCheck.y < gridLength.y && tileToCheck.y >= 0)//skip it
                    {
                        tileBeingChecked = myList[(int)tileToCheck.x][(int)tileToCheck.y];
                        if (tileBeingChecked.IsWall == false && tileBeingChecked.HasBeenChanged == false)//makes sure that the tiles with a lower values are not added to the search if they have alrady been checked
                        {
                            if (myList[(int)cT.x][(int)cT.y].EstimatedCost > tileBeingChecked.EstimatedCost || tileBeingChecked.HasBeenChanged == false)//checks if the Heuristic value of thetile being checked is less than the current tile Heuristic value
                            {
