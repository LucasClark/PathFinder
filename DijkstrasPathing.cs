...
{
    class DijkstrasPathing : PathFinder
    {
        public List<Tile> FindPath(List<List<TileObject>> myList, TileObject startPoint, TileObject exitPoint, Vector2 gridLength)
        {
            ...
            while (stopper < stop)//while there are unchecked nodes or while the stoppper is below its allowed limit
            {
                ...
                for (int i = 0; i < searchList.Count; i++)//if searchlist[i] has a lower cost then the current searchRef tile that tile is now the searchRef tile
                    if (searchList[i].Cost < searchList[searchRef].Cost)
                    {
                        searchRef = i;
                    }
